...
public class SeoApiController : ControllerBase
{
    ...
    [ProducesResponseType(404)]
    public async Task<ActionResult<SeoBatchJob>> GetBatchJob(long jobId)
    {
        var job = await _templateService.GetBatchJobAsync(jobId);
        if (job == null)
        {
            return NotFound(new { error = $"Batch job with ID {jobId} not found" });
        }
    ...
    [ProducesResponseType(404)]
    public async Task<ActionResult<ContentMetadata>> GetContentMetadata(string contentId, [FromQuery] string contentType = "movie")
    {
        var metadata = await _contentMetadataService.GetContentMetadataAsync(contentId, contentType);
        if (metadata == null)
        {
    ...
    [ProducesResponseType(typeof(List<TrendingContent>), 200)]
    public async Task<ActionResult<List<TrendingContent>>> GetTrendingContent(
    ...
    /// </summary>
    [HttpGet("analytics")]
    [ProducesResponseType(typeof(SeoAnalyticsResponse), 200)]
    public async Task<ActionResult<SeoAnalyticsResponse>> GetAnalytics(
    ...
    /// <summary>
    /// Demonstrate the complete content generation system
    /// Creates sample templates and pages to verify the system works
    /// </summary>
    [HttpPost("demo/generate")]
    [ProducesResponseType(typeof(Dictionary<string, object>), 200)]
    [ProducesResponseType(typeof(object), 500)]
    public async Task<ActionResult<Dictionary<string, object>>> RunContentGenerationDemo()
    {
        try
        {
            _logger.LogInformation("Starting content generation demonstration");
            
            var demo = new GeoLeap.Api.Demo.ContentGenerationDemo(HttpContext.RequestServices);
            var result = await demo.DemonstrateContentGenerationAsync();
            
            var response = result.GetSummary();
            response["details"] = new
            {
                templateId = result.TemplateId,
                pagesGenerated = result.GeneratedPages.Select(p => new
                {
                    id = p.Id,
                    slug = p.Slug,
                    title = p.MetaTitle,
                ...
                batchPagesRequested = result.BatchPagesRequested,
                duration = result.Duration.TotalMilliseconds
            };
            
            if (result.Success)
            {
                _logger.LogInformation("Content generation demo completed successfully: {Message}", result.Message);
                return Ok(response);
            }
            else
            {
                _logger.LogWarning("Content generation demo failed: {Message}", result.Message);
                return StatusCode(500, response);
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Content generation demo failed with exception");
            return StatusCode(500, new { error = "Demo failed", message = ex.Message });
        }
    }
