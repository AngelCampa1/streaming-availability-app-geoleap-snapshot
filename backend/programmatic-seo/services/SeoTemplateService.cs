...
public class SeoTemplateService : ISeoTemplateService
{
    ...
    public async Task<List<string>> GetTemplateErrorsAsync(SeoTemplate template)
    {
        ...
        try
        {
            ...
            if (!_fluidParser.TryParse(template.Template, out var fluidTemplate, out var fluidErrors))
            {
                errors.AddRange(fluidErrors.Select(e => $"Template syntax error: {e}"));
            }
            
            // Validate meta title template
            if (!string.IsNullOrEmpty(template.MetaTitle) && 
                !_fluidParser.TryParse(template.MetaTitle, out _, out var metaTitleErrors))
            {
                errors.AddRange(metaTitleErrors.Select(e => $"Meta title error: {e}"));
            }
            
            // Validate meta description template
            if (!string.IsNullOrEmpty(template.MetaDescription) && 
                !_fluidParser.TryParse(template.MetaDescription, out _, out var metaDescErrors))
            {
                errors.AddRange(metaDescErrors.Select(e => $"Meta description error: {e}"));
            }
            
            // Validate H1 template
            if (!string.IsNullOrEmpty(template.H1Template) && 
                !_fluidParser.TryParse(template.H1Template, out _, out var h1Errors))
            {
                errors.AddRange(h1Errors.Select(e => $"H1 template error: {e}"));
            }
    ...
    public async Task<Dictionary<string, object>> ParseTemplateVariablesAsync(string templateContent)
    {
        var variables = new Dictionary<string, object>();
        
        // Parse Liquid template to extract variables
    ...
    public async Task<SeoBatchJob> StartBatchJobAsync(SeoBatchGenerationRequest request)
    {
        ...
        _ = Task.Run(async () => await ProcessBatchJobAsync(job.Id, request));
    ...
    public async Task<SeoBatchJob?> GetBatchJobAsync(long jobId)
    {
        return await _context.SeoBatchJobs
    ...
    public async Task<string> RenderTemplateAsync(string templateContent, Dictionary<string, object> variables)
    {
        try
        {
            ...
            // Parse template
            if (!_fluidParser.TryParse(templateContent, out var template))
            {
    ...
    private void ConfigureFluidTemplates()
    {
        // Add custom filters for SEO
        _templateOptions.Filters.AddFilter("slugify", (input, arguments, context) => 
            SlugifyUrl(input.ToStringValue()));
        
        _templateOptions.Filters.AddFilter("truncate", (input, arguments, context) =>
        {
            var length = arguments.At(0).ToNumberValue();
            return TruncateText(input.ToStringValue(), (int)length);
        });
        
        _templateOptions.Filters.AddFilter("capitalize_each", (input, arguments, context) =>
            CapitalizeEachWord(input.ToStringValue()));
    }
    ...
    /// </summary>
    public async Task<string> InjectInternalLinksAsync(string content, List<InternalLink> links)
    {
    ...
    private async Task<string> GenerateLinkTextAsync(SeoPage sourcePage, SeoPage targetPage, Dictionary<string, object> variables)
    {
        ...
        {
            return $"{targetVariables["title"]} ({targetVariables["year"]})";
        }
        
        if (targetVariables.ContainsKey("genre") && targetVariables.ContainsKey("country"))
        {
            return $"{targetVariables["genre"]} movies in {targetVariables["country"]}";
        }
        
        // Fallback to meta title
        return targetPage.MetaTitle ?? targetPage.H1 ?? "Related content";
    }
    ...
    public async Task<Dictionary<string, object>> GetTemplatePerformanceAsync(int templateId, DateTime? startDate = null, DateTime? endDate = null)
    {
    ...
    public async Task<SeoAnalyticsResponse> GetAnalyticsAsync(DateTime? startDate = null, DateTime? endDate = null)
    {
        var start = startDate ?? DateTime.UtcNow.AddDays(-30);
        var end = endDate ?? DateTime.UtcNow;
        
        var totalPages = await _context.SeoPages.CountAsync();
        var publishedPages = await _context.SeoPages.CountAsync(p => p.IsPublished);
        var totalViews = await _context.SeoPages.SumAsync(p => p.ViewCount);
        
        var topPages = await _context.SeoPages
            .Where(p => p.IsPublished && p.GeneratedAt >= start && p.GeneratedAt <= end)
            .OrderByDescending(p => p.ViewCount)
            .Take(10)
            .Select(p => new TopPerformingPage
            {
                PageId = p.Id,
                Slug = p.Slug,
                Title = p.MetaTitle ?? p.H1 ?? "Untitled",
                Views = p.ViewCount,
                Position = p.SearchRanking ?? 0,
                ClickThroughRate = 0 // Would calculate from analytics
            })
            .ToListAsync();
        ...
        return new SeoAnalyticsResponse
        {
            ...
            UniqueVisitors = (int)(totalViews * 0.7), // Estimate
            TopPages = topPages
        };
    }
    
    public async Task<List<TopPerformingPage>> GetTopPerformingPagesAsync(int count = 10)
    {
        return await _context.SeoPages
    ...
    public async Task<List<SeoPage>> GetUnderperformingPagesAsync(int count = 10)
    {
        // Implementation for underperforming pages
        return new List<SeoPage>();
    }
    
    public async Task<Dictionary<string, object>> GetContentGapsAsync()
    {
        // Implementation for content gap analysis
        return new Dictionary<string, object>();
    }
    
    public async Task<int> RefreshOutdatedPagesAsync(int batchSize = 100)
    {
        var cutoffDate = DateTime.UtcNow.AddDays(-7); // Pages older than 7 days
        var outdatedPages = await _context.SeoPages
            .Where(p => p.LastUpdated < cutoffDate || (p.LastUpdated == null && p.GeneratedAt < cutoffDate))
            .Take(batchSize)
            .ToListAsync();
        
        int refreshedCount = 0;
        foreach (var page in outdatedPages)
        {
            try
            {
                ...
                if (template != null)
                {
                    var newContent = await RenderTemplateAsync(template.Template, variables);
            ...
            {
                _logger.LogWarning(ex, "Failed to refresh page {PageId}", page.Id);
            }
        }
    ...
    public async Task<int> BulkUpdatePagesAsync(int templateId, Dictionary<string, object> globalUpdates)
    {
    ...
    public async Task<string> GenerateXmlSitemapAsync(List<int>? templateIds = null)
    {
        var query = _context.SeoPages.Where(p => p.IsPublished);
        
        if (templateIds?.Any() == true)
        {
            query = query.Where(p => templateIds.Contains(p.TemplateId));
        }
        
        var pages = await query
            .OrderBy(p => p.Slug)
            .Select(p => new { p.Slug, p.LastUpdated, p.GeneratedAt })
            .ToListAsync();
        ...
        foreach (var page in pages)
        {
            ...
            sitemap.AppendLine($"    <loc>https://geoleap.app/seo/pages/{page.Slug}</loc>");
            
            var lastModified = page.LastUpdated ?? page.GeneratedAt;
            sitemap.AppendLine($"    <lastmod>{lastModified:yyyy-MM-dd}</lastmod>");
    ...
    public async Task<Dictionary<string, object>> GetSitemapStatsAsync()
    {
        var totalPages = await _context.SeoPages.CountAsync();
        var publishedPages = await _context.SeoPages.CountAsync(p => p.IsPublished);
        var templatesWithPages = await _context.SeoPages
        ...
        return new Dictionary<string, object>
        {
            ["totalPages"] = totalPages,
            ["publishedPages"] = publishedPages,
            ["templatesWithPages"] = templatesWithPages,
            ["lastGenerated"] = DateTime.UtcNow
        };
    }
    
    public async Task<List<SeoPage>> DetectDuplicateContentAsync(float similarityThreshold = 0.8f)
    {
        // Implementation for duplicate content detection
        return new List<SeoPage>();
    }
    
    public async Task<List<string>> ValidateGeneratedContentAsync(long pageId)
    {
    ...
    public async Task<bool> CheckContentUniquenessAsync(string content, int? excludePageId = null)
    {
