 ...
 *
 * OPTIMIZED: Uses singleton NetworkService instance to prevent memory leaks
 */
 ...
 * OPTIMIZED: Uses singleton instance and proper cleanup
 */
export function useNetworkStatus(options: UseNetworkStatusOptions = {}) {
  ...
  // Use singleton instance (memoized, won't change between renders)
  ...
  const testConnection = useCallback(async (): Promise<boolean> => {
    setIsTesting(true);
    try {
      const isHealthy = await networkService.testConnection();
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      unsubscribeStatus = networkService.onConnectionChange(updateStatus);

      // Setup quality listener
      unsubscribeQuality = networkService.onQualityChange(updateQuality);
