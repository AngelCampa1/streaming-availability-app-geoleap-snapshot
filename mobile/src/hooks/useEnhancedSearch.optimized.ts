/**
 * Enhanced Search Hook with Performance Optimizations
 * OPTIMIZED: Includes AbortController for race condition prevention
 */
...
export const useEnhancedSearch = (options: UseEnhancedSearchOptions = {}): UseEnhancedSearchReturn => {
  ...
  const {
    ...
    queryFn: async ({ pageParam }: { pageParam: number }) => {
      ...
      try {
        // Pass abort signal to search service (removed - not supported)
        const results = await searchService.search(searchQuery);
        return results;
      } catch (error) {
        // Handle abort gracefully
        if ((error as Error).name === 'AbortError') {
          logger.log('[useEnhancedSearch] Search request cancelled');
    ...
    gcTime: cacheEnabled ? 10 * 60 * 1000 : 0, // 10 minutes
  });
  ...
  // Search suggestions query with AbortController
  const {
  ...
  // CRITICAL: Cleanup timeouts and abort controllers on unmount
  useEffect(() => {
    return () => {
      // Cancel any pending requests
      if (abortControllerRef.current) {
