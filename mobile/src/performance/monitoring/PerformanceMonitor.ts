...
class PerformanceMonitor {
  ...
  private async getBundleSize(): Promise<number> {
    try {
      ...
      if (bundleStats) {
        const _stats = JSON.parse(bundleStats);
        return _stats.size / (1024 * 1024); // Convert to MB
      }
  ...
  private reportAlerts(alerts: PerformanceAlert[]): void {
    alerts.forEach(alert => {
      logger.warn(`[PerformanceMonitor] Performance Alert [${alert.severity.toUpperCase()}]`, {
